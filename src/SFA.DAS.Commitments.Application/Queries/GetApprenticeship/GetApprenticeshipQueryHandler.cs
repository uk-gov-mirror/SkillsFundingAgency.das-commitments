            ...
            switch (message.Caller.CallerType)
            {
                case CallerType.Provider:
                    if (apprenticeship.ProviderId != message.Caller.Id)
                        throw new UnauthorizedException(ApiExceptionIds.AccessDeniedForProvider, $"Provider {message.Caller.Id} not authorised to access apprenticeship {message.ApprenticeshipId}, expected provider {apprenticeship.ProviderId}");
                    break;
                case CallerType.Employer:
                default:
                    if (apprenticeship.EmployerAccountId != message.Caller.Id)
                        throw new UnauthorizedException(ApiExceptionIds.AccessDeniedForEmployer, $"Employer {message.Caller.Id} not authorised to access apprenticeship {message.ApprenticeshipId}, expected employer {apprenticeship.EmployerAccountId}");
                    break;
            }
        }
    }
