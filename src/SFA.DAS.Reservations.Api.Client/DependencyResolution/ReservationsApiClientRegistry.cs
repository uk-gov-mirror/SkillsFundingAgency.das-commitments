...
namespace SFA.DAS.Reservations.Api.Client.DependencyResolution
{
    public class ReservationsApiClientRegistry : Registry
    {
        ...
        private IReservationsApiClient CreateClient(IContext ctx)
        {
            ...
            if (config.UseStub)
            {
                httpClient = new HttpClient {BaseAddress = new System.Uri("http://localhost:3203/")};
                //httpClient.BaseAddress = new System.Uri("https://sfa-stub-reservations.herokuapp.com");
            }
            else
            {
